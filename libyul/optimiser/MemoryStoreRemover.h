...
private:
	...
	/// Given `_instruction` along with its arguments, and a variable, say `t`, representing a
	/// memory location. Suppose that the `_instruction` modifies the memory locations `[r, s)`.
	/// This function would add the constraint:
	///     `r <= t < s`
	/// to the solver.
