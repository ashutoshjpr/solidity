...
/// Helper function that returns the instruction, if the function
optional<Instruction> toInstruction(
	YulString const& _name,
	Dialect const& _dialect
)
{
	if (auto dialect = dynamic_cast<EVMDialect const*>(&_dialect))
		if (auto builtin = dialect->builtin(_name))
...
void MemoryStoreRemover::operator()(FunctionCall const& _functionCall)
{
	...
	else if (auto instruction = toInstruction(_functionCall.functionName.name, m_dialect))
...
)
{
	...
	optional<Instruction> instruction = toInstruction(_functionCall.functionName.name, m_dialect);
